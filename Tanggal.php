...
class Tanggal {
    ...
    public static function toReadableED($date, $withSpan=TRUE, $forceDate=FALSE){
        if($forceDate){
            $date = date("Y-m-d", strtotime($date));
        }
        ...
        $time = strtotime($date);
        $padTime = str_pad($time, 12, "0", STR_PAD_LEFT);
        return ($withSpan?"<span style='display:none'>{$padTime}</span>":"").($withHour?Tanggal::getBulan(date($time)).date(" Y, H:i", $time):Tanggal::getBulan(date($time)).date(" Y", $time));
    }

    private static function getBulan($time){
